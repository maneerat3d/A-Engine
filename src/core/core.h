#pragma once
	...
	#ifdef AENGINE_DEBUG
		#ifdef _WIN32
			#define AENGINE_DEBUG_BREAK() __debugbreak()
		#else
			#define AENGINE_DEBUG_BREAK()  raise(SIGTRAP) 
		#endif
		#define ASSERT(x) do { const volatile bool lumix_assert_b____ = !(x); if(lumix_assert_b____) AENGINE_DEBUG_BREAK(); } while (false)
	#else
...
#ifdef _WIN32
	#define AENGINE_LIBRARY_EXPORT __declspec(dllexport)
	#define AENGINE_LIBRARY_IMPORT __declspec(dllimport)
	#define AENGINE_FORCE_INLINE __forceinline
	#define AENGINE_RESTRICT __restrict
#else 
