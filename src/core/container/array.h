#pragma once
#include <cstring>      // สำหรับ memcpy, memmove
#include <type_traits>  // สำหรับ __is_trivially_copyable
...
template <typename T> struct Array {
	...
	void copyTo(Array<T>& dst) const {
		...
		if constexpr (std::is_trivially_copyable_v<T>) { // ใช้ std::is_trivially_copyable_v
			dst.resize(m_size);
